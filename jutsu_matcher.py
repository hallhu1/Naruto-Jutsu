...
def draw_landmarks_on_image(rgb_image, detection_result):
  ...
  for idx in range(len(hand_landmarks_list)):
    ...
    for landmark in hand_landmarks:
      # Draw the (x, y) value of each coordinate on the image
      cx = int(landmark.x * width)
      cy = int(landmark.y * height)

      coord_text = f"({landmark.x:.2f}, {landmark.y:.2f})"

      cv2.putText(
          annotated_image,
          coord_text,
          (cx + 5, cy - 5),           # offset text slightly so it’s visible
...
def run_jutsu_matcher(image_name: str):
  """
